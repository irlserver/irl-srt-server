...
// sls_parse_ts_info (which drives the PAT / PMT / PES path). Because libFuzzer
// poisons the bytes immediately after `data[size-1]`, any regression to an
// unbounded read trips AddressSanitizer and the fuzzer records a crash. It adds
// NO parser logic.
#include <climits>
#include <cstddef>
#include <cstdint>
...
{
    // sls_parse_ts_info takes an int length; a corpus file larger than INT_MAX
    // would wrap negative. Skip it (the parser would reject a negative length
    // anyway), so the narrowing cast is well-defined.
